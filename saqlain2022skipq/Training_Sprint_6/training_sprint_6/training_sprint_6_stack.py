...
class TrainingSprint6Stack(Stack):
    ...
    def __init__(self, scope: Construct, construct_id: str, **kwargs) -> None:
        ...
        ''' ********** SPRINT 6 built images to Elastic Container Registry (ECR) ********** '''
        self.ecr_repo = self.create_ecr_repository()
        ...
        duration_alarm.apply_removal_policy(RemovalPolicy.DESTROY)
        ...
        ConcurrentExecutions_alarm = cw.Alarm(self, "ConcurrentExecutionsExceed",
            ...
            threshold=1,
            comparison_operator=cw.ComparisonOperator.GREATER_THAN_THRESHOLD)
        ConcurrentExecutions_alarm.apply_removal_policy(RemovalPolicy.DESTROY)
        ...
        AsyncEventAge_alarm = cw.Alarm(self, "AsyncEventAgeExceed",
            ...
            threshold=30,
            comparison_operator=cw.ComparisonOperator.GREATER_THAN_THRESHOLD)
        AsyncEventAge_alarm.apply_removal_policy(RemovalPolicy.DESTROY)
        ...
        for i in range(len(URLs_Monitor)):
            ...
            availability_alarm.apply_removal_policy(RemovalPolicy.DESTROY)
            ...
            latency_alarm.apply_removal_policy(RemovalPolicy.DESTROY)
    ...
    #https://docs.aws.amazon.com/cdk/api/v2/python/aws_cdk.aws_ecr/README.html
    def create_ecr_repository(self):
        return ecr.Repository(self, "SaqlainRepository",
                            image_scan_on_push=True,
                            image_tag_mutability= ecr.TagMutability.IMMUTABLE,
                            lifecycle_rules=[
                                # https://docs.aws.amazon.com/cdk/api/v2/python/aws_cdk.aws_ecr/LifecycleRule.html
                                ecr.LifecycleRule(
                                    description= "api_pyresttest",
                                    max_image_count  = 2,
                                    rule_priority = 1
                                )
                            ],
    ...
    def create_and_deploy_image(self):
        ...
        image = ecra.DockerImageAsset(self, "saqlain_ecr",
                directory="./pyresttest"    
        )

        # https://docs.aws.amazon.com/cdk/api/v2/python/aws_cdk.aws_ecr_assets/README.html#publishing-images-to-ecr-repositories
        # https://github.com/cdklabs/cdk-ecr-deployment
        ecrdeploy.ECRDeployment(
        self,
        'basic_api_test',
        src = ecrdeploy.DockerImageName(image.image_uri),
        dest = ecrdeploy.DockerImageName(f"{self.ecr_repo.repository_uri}:latest")
        )
